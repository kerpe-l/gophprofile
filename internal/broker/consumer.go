...
// неподдерживаемый формат, удалённый аватар. Обработчик оборачивает в неё
// то, что заведомо неисправимо; всё остальное считается повторяемым, потому
// что лишний прогон по лестнице дешевле потерянного аватара.
var ErrNonRetryable = errors.New("non-retryable error")

// Message — доставленное событие.
type Message struct {
	// Type — тип события: RoutingKeyUploaded или RoutingKeyDeleted.
	Type string
	// MessageID — идентификатор сообщения.
	MessageID string
	// Body — тело события в JSON.
	Body []byte
	// Attempt — номер текущей попытки обработки, считая с единицы.
	Attempt int
	// Final — повтора после провала этой попытки не будет: сообщение уйдёт
	// в очередь мёртвых. Обработчику это нужно, чтобы проставить записи
	// терминальный статус, — про исчерпание попыток знает только консьюмер,
	// а про базу только обработчик.
	Final bool
}
...
type Consumer struct {
	...
	prefetch int
	// timeout — предел на обработку одного сообщения.
...
// или соединения — ошибка: молча переставший потреблять воркер выглядит
// живым, но не делает ничего.
