...
func (c *Conn) Ping(ctx context.Context) error {
	...
	// и отменяет только ожидание: по тайм-ауту Ping возвращается, а горутина
	// живёт до конца операции на канале или до разрыва соединения по heartbeat.
	// Отправка в done не блокирует её — канал буферизован. Так что при частых
	// health-check'ах и медленном брокере горутины копятся, но не дольше окна
	// heartbeat; это принимается осознанно: отменить операцию нечем, а закрывать
	// канал из чужой горутины ради отмены — хуже, чем переждать.
	go func() {
		done <- c.checkQueue()
	}()
