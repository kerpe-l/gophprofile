...
//
// Счётчик попыток консьюмер ведёт сам: историю отклонений в заголовке x-death
// брокер собирает заново на каждом попадании в очередь и выставленную клиентом
// не сохраняет, поэтому у сообщения, которое перекладывают со ступени на
// ступень, она всегда показывает одно отклонение. История нужна только там,
// где своего счётчика ещё нет: сообщение, отклонённое основной очередью,
// приходит с историей, но без счётчика, и первой попыткой уже не является.
...
// Записи неожиданного вида пропускаются: заголовок собирает брокер, и падать
// из-за расхождения в его формате обработке незачем — худшее, что даст
// пропуск, это лишний прогон по лестнице.
