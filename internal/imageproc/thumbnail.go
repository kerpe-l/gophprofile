...
// Thumbnails создаёт миниатюры оригинала во всех запрошенных размерах
// и возвращает их готовыми к загрузке в хранилище: JPEG качества из
// конфигурации, тип содержимого — ThumbnailMimeType.
//
// Изображение декодируется один раз на все размеры и кадрируется по центру
// с сохранением пропорций, поэтому результат всегда точно запрошенного
// размера. Прозрачность схлопывается в белый фон: JPEG альфа-канала не имеет.
//
...
//
// Прочитанное складывается в буфер и склеивается обратно с остатком потока:
// перемотать источник нельзя, а перечитывать его из хранилища ради каждой
// проверки — лишний запрос. В памяти при этом оседает только шапка файла.
...
// flatten накладывает изображение на белый фон.
//
// JPEG не хранит альфа-канал, и без явной подложки энкодер stdlib берёт
// у полностью прозрачного пикселя его цвет — у PNG это обычно чёрный,
// и прозрачный фон превращается в чёрную рамку.
