...
// затем размеры из заголовка. Без этого 10 MB PNG разворачивается
// в 30000×30000 и кладёт процесс по памяти раньше, чем сработает лимит
// на размер файла.
//
// Все ошибки пакета — доменные и неисправимые: повтор через пять минут
// не сделает битый JPEG целым.
package imageproc
...
// кроме jpeg, png и webp.
//
// Расширение имени файла и заголовок Content-Type от клиента не участвуют:
// и то и другое задаёт загружающий, и совпадение с содержимым ничем
// не гарантировано.
...
// isAnimatedWebP сообщает, что файл — анимированный WebP.
//
// Проверка нужна отдельно: image.DecodeConfig на таком файле отдаёт размеры
// первого кадра и никакой ошибки, а разваливается только полное
// декодирование — то есть уже в обработчике, после успешной загрузки.
